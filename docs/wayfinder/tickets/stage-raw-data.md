...
- Stored timestamp offsets are mixed per source: EURUSD 1m uses `+0100`, XAUUSD and DXY 1m use `-0400`, daily bars use `+00:00`. The read path parses to UTC and is safe, but string range filters in SQL compare raw text and will misorder across offsets. Cleaning must normalize before any string comparison.
